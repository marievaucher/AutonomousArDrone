...
class UserCode:
    ...
    def predictState(self, dt, x, u_linear_velocity, u_yaw_velocity):
        ...
        # the control inputs local linear velocity and yaw velocity using velocity sensors (very noisy)
        # accumulate drift but correct with marker observation
    ...
    def correctCovariance(self, sigma_p, K, H):
        ...
        # corrects the sate covariance matrix using Kalman gain and the Jacobian matrix of the
        # predictMeasurement(...) function

        return np.dot(np.identity(3) - np.dot(K, H), sigma_p)

    def predictMeasurement(self, x, marker_position_world, marker_yaw_world):

        # predicts a marker measurement given the current state and the marker position and
        # orientation in world coordinates

        z_predicted = Pose2D(self.rotation(x[2]), x[0:2]).inv() * Pose2D(self.rotation(marker_yaw_world),
    ...
    def calculatePredictMeasurementJacobian(self, x, marker_position_world, marker_yaw_world):
        ...
        # calculates the 3x3 Jacobian matrix of the predictMeasurement(...) function using
    ...
    def measurement_callback(self, marker_position_world, marker_yaw_world, marker_position_relative,
                             marker_yaw_relative):

        # called when a new marker measurement arrives max 30Hz, marker measurements are only available
        # if the quadrotor is
...
class Pose2D:
    ...
    def __mul__(self, other):
        ...
        return Pose2D(np.dot(self.rotation, other.rotation),
                      np.dot(self.rotation, other.translation) + self.translation)